...
const countryCodeModel = require("../models/countryCodeModel");
...
const countryList = async (req, res) => {
    try {
        ...
        if (search) {
            ...
            or[1] = { phoneCode: { $regex: search, $options: "i" } }

            q.$or = or;
        }
        let list = await countryModel.find(q)
        return res.status(200).send({ status: true, message: "success!", data: list })
...
const stateList = async (req, res) => {
    try {
        ...
        if (stateName) { stateName = stateName.charAt(0).toUpperCase() + stateName.slice(1); }
        let list = await countryModel.findOne({ name })
        let StateList = State.getStatesOfCountry(list?.countryCode)
        if (stateName) {
            const regex = new RegExp(stateName, "i");
            const matchedCities = StateList.filter(city => regex.test(city.name));
            return res.status(200).send({ status: true, message: "success!", data: matchedCities })
        }
        else {
            return res.status(200).send({ status: true, message: "success!", data: StateList })
        }
    } catch (error) {
        console.log(error);
...
const cityList = async (req, res) => {
    try {
        let { countryCode, isoCode, name } = req.body;

        if (name) { name = name.charAt(0).toUpperCase() + name.slice(1); }

        let CityList = city.getCitiesOfState(countryCode, isoCode)

        if (name) {
            const regex = new RegExp(name, "i");
            const matchedCities = CityList.filter(city => regex.test(city.name));
            return res.status(200).send({ status: true, message: "success!", data: matchedCities })
        }
        else {

            return res.status(200).send({ status: true, message: "success!", data: CityList })
        }
...
const religionList = async (req, res) => {
    try {

        let create = await religionModel.find()
        return res.status(200).send({ status: true, message: "success!", data: create })
...
const casteList = async (req, res) => {
    try {
        let { name, casteName } = req.body;
        name = name.charAt(0).toUpperCase() + name.slice(1);
        if (casteName) { casteName = casteName.charAt(0).toUpperCase() + casteName.slice(1); }
        let find = await religionModel.findOne({ name: name })
        var Name = find.name
        let obj = []
        religion.map((i) => {
            nm = i.name
            ...
            if (nm == Name) {
                Caste.map((j) => {
                    caste = j.name
                    obj.push({ caste: caste })
                })
            }
        })
        if (casteName) {
            const regex = new RegExp(casteName, "i");
            const matchedCities = obj.filter(caste => regex.test(caste.caste));
            return res.status(200).send({ status: true, message: "success!", data: matchedCities })
        } else {
...
const listCountryCode = async (req, res) => {
    try {
        let search = req.query.q;
        let q = {}
        if (search) {
            let or = [];
            or[0] = { country_Code: { $regex: search, $options: "i" } }
            q.$or = or;
        }
        let list = await countryCodeModel.find(q)
        return res.status(200).send({ status: true, message: "success!", data: list })
...
module.exports = {
    countryList, stateList, cityList, religionList, casteList, listCountryCode
}